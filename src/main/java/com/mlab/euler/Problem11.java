...
public class Problem11 implements Problem {
	...
	private static final int[][] data = {   {8,2,22,97,38,15,0,40,0,75,4,5,7,78,52,12,50,77,91,8},
	...
	private int rowProduct(int i, int j) {
		int product = 1;
	...
	private int colProduct(int i, int j) {
		int product = 1;
		for(int k = 0; k < NUM_PRODUCT; k++) {
			product *= data[i][j + k];
		}
	...
	private int rightDiagProduct(int i, int j) {
		int product = 1;
		for(int k = 0; k < NUM_PRODUCT; k++) {
	...
	private int leftDiagProduct(int i, int j) {
		i += NUM_PRODUCT - 1;
		int product = 1;
		for(int k = 0; k < NUM_PRODUCT; k++) {
			product *= data[i - k][j + k];
	...
	public void solve() {
		for(int i = 0; i < data.length - (NUM_PRODUCT - 1); i++) {
			for(int j = 0; j < data[i].length - (NUM_PRODUCT - 1); j++) {
				...
				maxProduct = Math.max(maxProduct, colProduct(i, j));
				maxProduct = Math.max(maxProduct, rightDiagProduct(i, j));
